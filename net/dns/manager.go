...
func (m *Manager) compileConfig(cfg Config) (rcfg resolver.Config, ocfg OSConfig, err error) {
	...
	case cfg.hasDefaultIPResolversOnly():
		...
		// resolver.
		// TODO: for OSs that support it, pass IP:port and DoH
	...
	if !m.os.SupportsSplitDNS() || isWindows {
		...
		for _, ip := range bcfg.Nameservers {
			defaultRoutes = append(defaultRoutes, dnstype.Resolver{Addr: netaddr.IPPortFrom(ip, 53).String()})
		}
...
// DoH or custom-port entries with somethnig like hasDefaultIPResolversOnly.
